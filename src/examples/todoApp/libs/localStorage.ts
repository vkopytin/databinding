...
class LocalStorage {
    name: string;
    records;
    serializer: { serialize(item); deserialize(data); };
    constructor(name: string, serializer = {
        serialize(item) {
...
function localSync(method, model, options) {
    ...
    const syncDfd = BB.$.Deferred && BB.$.Deferred();
...
const ajaxSync = BB.sync;
  
function getSyncMethod(model, options) {
    const forceAjaxSync = options && options.ajaxSync;
...
BB['sy' + 'nc'] = function (method, model, options) {
